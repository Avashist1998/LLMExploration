...
class NumberGenerator:
    ...
    def generate_number(self, min_val: float, max_val: float, prompt_type: str = "direct") -> Optional[float]:
        """
        ...
        try:
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are a precise number generator. Always respond with only the requested number."},
            ...
            try:
                ...
                if min_val <= number <= max_val:
                    ...
                    print(f"Warning: Generated number {number} outside range [{min_val}, {max_val}]")
                    return None
    ...
    def generate_batch(self, min_val: float, max_val: float, count: int, 
                      prompt_type: str = "direct", delay: float = 0.1) -> List[float]:
        """
    ...
    def run_consistency_test(self, ranges: List[Tuple[float, float]], 
        ...
        for run in range(runs):
            ...
            for min_val, max_val in ranges:
                ...
                print(f"  Generating {samples_per_range} numbers in range [{min_val}, {max_val}]...")
                
                numbers = self.generate_batch(min_val, max_val, samples_per_range, prompt_type)
