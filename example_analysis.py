...
def compare_models():
    ...
    for model, model_results in results.items():
        ...
        for range_key, range_analysis in analysis["range_analysis"].items():
            ...
            print(
                f"  {range_key}: bias={bias:.3f}, coverage={coverage:.3f}, uniform={uniformity}"
            )
...
def analyze_specific_bias():
    ...
    for range_key, range_analysis in analysis["range_analysis"].items():
        ...
        print(
            f"  {range_key}: {bias_direction} bias ({bias:.3f}), coverage={coverage:.3f}, uniform={uniformity}"
        )
...
def test_consistency_over_time():
    ...
    for range_key, consistency in analysis["consistency_analysis"].items():
        ...
        consistency_level = (
            "high" if cv_mean < 0.1 else "medium" if cv_mean < 0.2 else "low"
        )
...
def create_custom_visualization():
    ...
    bars1 = ax1.bar(
        range_keys, biases, color=["red" if b > 0 else "blue" for b in biases]
    )
    ...
    ks_p_values = [
        analysis["range_analysis"][r]["uniformity_test"]["ks_p_value"]
        for r in range_keys
    ]
