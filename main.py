...
def main():
    ...
    try:
        ...
        print(f"   - Total numbers generated: {sum(len(run_data[range_key]) for run_key, run_data in results['data'].items() for range_key in run_data)}")
        ...
        print(f"   - Bias standard deviation: {analysis['bias_analysis']['bias_std']:.4f}")
        ...
        for range_key, range_analysis in analysis['range_analysis'].items():
            ...
            print(f"     - KS test uniform: {uniformity['is_uniform_ks']} (p={uniformity['ks_p_value']:.4f})")
            print(f"     - Chi² test uniform: {uniformity['is_uniform_chi2']} (p={uniformity['chi2_p_value']:.4f})")
        ...
        analyzer.create_visualizations(results, analysis, f"analysis_results_{model}")
        
        # Save results
        print(f"\n💾 Saving results...")
        analyzer.save_results(results, analysis, f"analysis_results_{model}.json")
        
        print(f"\n✅ Analysis completed successfully!")
        print(f"   - Visualizations saved as: analysis_results_{model}_visualizations.png")
        print(f"   - Data saved as: analysis_results_{model}.json")
        print(f"   - Run 'python plot_results.py' to generate detailed distribution plots")
...
def run_quick_test():
    ...
    try:
        ...
        print(f"Running quick test with {samples_per_range} samples per range, {runs} runs...")
