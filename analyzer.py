...
class NumberAnalyzer:
    ...
    def analyze_distribution(self, results: Dict) -> Dict:
        ...
        for min_val, max_val in ranges:
            ...
            if all_numbers:
                ...
                expected_std = (max_val - min_val) / np.sqrt(12)  # For uniform distribution
                ...
                analysis['range_analysis'][range_key] = {
                    ...
                    'range_coverage': float((np.max(numbers_array) - np.min(numbers_array)) / (max_val - min_val)),
                    'uniformity_test': self._test_uniformity(numbers_array, min_val, max_val)
                }
        
        # Analyze bias patterns across ranges
        analysis['bias_analysis'] = self._analyze_bias_patterns(analysis['range_analysis'])
    ...
    def _test_uniformity(self, numbers: np.ndarray, min_val: float, max_val: float) -> Dict:
        """
        ...
        expected = len(normalized) / 10  # Expected count per bin for uniform distribution
    ...
    def _analyze_consistency(self, data: Dict, ranges: List[Tuple[float, float]]) -> Dict:
        """
        ...
        for min_val, max_val in ranges:
            ...
            if run_means:
                consistency[range_key] = {
                    'mean_consistency': float(np.std(run_means)),  # Lower is more consistent
                    'std_consistency': float(np.std(run_stds)),
                    'cv_mean': float(np.std(run_means) / np.mean(run_means)),  # Coefficient of variation
    ...
    def create_visualizations(self, results: Dict, analysis: Dict, save_path: str = "analysis_results"):
        """
    ...
    def _plot_distributions(self, results: Dict, fig, rows: int, cols: int, pos: int):
        ...
        for min_val, max_val in ranges:
            ...
            if all_numbers:
                ax.hist(all_numbers, alpha=0.6, label=f'[{min_val}, {max_val}]', bins=20)
    ...
    def _plot_uniformity_tests(self, analysis: Dict, fig, rows: int, cols: int, pos: int):
        """Plot uniformity test results."""
        ax = plt.subplot(rows, cols, pos)
        
        range_analysis = analysis['range_analysis']
        ranges = list(range_analysis.keys())
        ks_p_values = [range_analysis[r]['uniformity_test']['ks_p_value'] for r in ranges]
        chi2_p_values = [range_analysis[r]['uniformity_test']['chi2_p_value'] for r in ranges]
    ...
    def save_results(self, results: Dict, analysis: Dict, filename: str = "analysis_results.json"):
        """
    ...
    def _create_summary(self, analysis: Dict) -> Dict:
        ...
        summary = {
            ...
            'most_biased_range': max(bias_analysis['bias_by_range'].items(), key=lambda x: abs(x[1]))[0],
            'least_consistent_range': min(consistency_analysis.items(), key=lambda x: x[1]['cv_mean'])[0],
            'uniformity_findings': {}
        }
